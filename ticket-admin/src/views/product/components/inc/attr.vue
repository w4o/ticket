<template>
  <div style="margin-top: 50px">
    <el-form ref="productAttrForm" :model="value" label-width="120px" style="width: 720px" size="small">
      ...
      <el-form-item v-if="hasAttrPic" label="属性图片：">
        <el-card shadow="never" class="cardBg">
          <div v-for="item in selectProductAttrPics">
            <span>{{ item.name }}:</span>
            <single-upload
              v-model="item.pic"
  ...
  methods: {
    ...
    refreshProductSkuList() {
      ...
      if (this.selectProductAttr.length === 1) {
        ...
        const values0 = this.selectProductAttr[0].values
        const values1 = this.selectProductAttr[1].values
        ...
        const values0 = this.selectProductAttr[0].values
        const values1 = this.selectProductAttr[1].values
        const values2 = this.selectProductAttr[2].values
        for (let i = 0; i < values0.length; i++) {
          if (values1.length === 0) {
            skuList.push({
              sp1: values0[i]
